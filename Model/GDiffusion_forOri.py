...
# from Models.interpretable_diffusion.transformer_season import Transformer
...
class RFC_Diff(nn.Module):
    def __init__(
        ...
        self.model = RFC(n_feat=feature_size,aug_times=1, n_channel=seq_length, n_layer_enc=n_layer_enc,
    ...
    def diffusion_out(self, x_start_ori, t,traintestflag, target, noise=None, padding_masks=None):
        ...
        if traintestflag == True:
            target_unnormal = self.dataset.unnormalize(model_out.cpu().detach().numpy())
            for item in target_unnormal:
                tmplen = len(item)
                for i in range(tmplen):
                    self.totalcountlyy = self.totalcountlyy + 1
                    if item[i][0] > item[i][1]:
                        self.greater1_total = self.greater1_total + 1
                    if item[i][0] < item[i][2]:
                        self.less_neg1_total = self.less_neg1_total + 1
                    if item[i][3] > item[i][1]:
                        self.greater1_total = self.greater1_total + 1
                    if item[i][3] < item[i][2]:
                        self.less_neg1_total = self.less_neg1_total + 1
                    if item[i][1] < item[i][2]:
                        self.highlowerror = self.highlowerror + 1
        ...
        if self.counttrain % 1000 == 0 or self.counttrain == 1 or traintestflag == True:
            ...
            if traintestflag:
                ...
                self.testcount_norm["aver_corre"] = self.testcount_norm["aver_corre_total"] / self.testcount_norm[
                    "aver_corre_count"]
