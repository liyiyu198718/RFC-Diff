...
from ModelUtils.utils import normalize_to_neg_one_to_one,unnormalize_to_zero_to_one,noise_mask
...
class CustomDataset(Dataset):
    def __init__(
        ...
        self.rawdata, self.scaler = self.read_data(data_root,data_withrelaship)
    ...
    @staticmethod
    def read_data(filepath,data_withrelaship=None):
        """Reads a single .csv
        """
        if data_withrelaship is None:
            df = pd.read_csv(filepath, header=0)
            data = df.values
        else:
            data = data_withrelaship
        print("data.shape")
        print(data.shape)
        scaler = MinMaxScaler()
